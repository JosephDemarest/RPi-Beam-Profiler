...
from libs.blurb import fullpath, about_message
...
class CameraSettings(wx.Dialog):
	...
	def __init__(self, parent, id, title):		
		...
		#init plot
		self.nBins = 64
		self.line, = self.ax_hist.plot(np.linspace(0,1023,self.nBins),np.zeros(self.nBins), \
							color=DialogOptions.linecol,lw=2)
		self.HistPanel.draw()		
	...
	def initUI(self):
		...
		ExpAutoCtrl.SetToolTip(wx.ToolTip("Automatically adjust exposure \
		...
		self.ColCtrl = wx.ComboBox(self,value=self.Col, 
			choices=ColChoices,style=wx.CB_READONLY, size=(120,-1))
		...
		self.histfig = plt.figure(2,(4,3),80,facecolor=(230./255,230./255,230./255))
	...
	def OnGenHist(self,event):
		...
		self.line.set_ydata(hdata.astype(np.float)/hdata.max())
		self.HistPanel.draw()				
	...
	def setCrop(self):
		...
		if self.ROIxminval < 0: self.ROIxminval = 0
		if self.ROIyminval < 0: self.ROIyminval = 0
		if self.ROIxmaxval > cam.ccd_xsize: self.ROIxmaxval = cam.ccd_xsize
		if self.ROIymaxval > cam.ccd_ysize: self.ROIymaxval = cam.ccd_ysize
...
class DialogDefaults:
	...
	crop = (ROIxminval, ROIxmaxval-ROIxminval, \
			ROIyminval, ROIymaxval-ROIyminval)
...
class ScanSettings(wx.Dialog):
	...
	def initUI(self): 
		...
		self.CurrentPosValue = wx.TextCtrl(self,value=str(round(self.parent.Stepper.get_position(),3)), \
							style=wx.TE_READONLY|wx.TE_RIGHT,size=(80,-1))
...
#		
	...
	def OnCamSet(self,event):
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			DialogOptions.crop = (dlg.ROIxminval, dlg.ROIxmaxval-dlg.ROIxminval, \
									dlg.ROIyminval, dlg.ROIymaxval-dlg.ROIyminval)
	...
	def update_main_imshow(self):
		...
		self.axX.set_ylim(min(0,1.1*self.xslice.get_ydata().min()),1.1*self.xslice.get_ydata().max())
		self.axY.set_xlim(min(0,1.1*self.xslice.get_ydata().min()),1.1*self.yslice.get_xdata().max())
		...
		if len(self.xposdata)>0:
			self.axXwidth.set_ylim(self.xwline.get_data()[1].min()*0.8, \
									self.xwline.get_data()[1].max()*1.1)
			self.axYwidth.set_ylim(self.ywline.get_data()[1].min()*0.8, \
									self.ywline.get_data()[1].max()*1.1)
		...
		#update plot text
		#self.imagefitparams #////
		self.xfit_text.set_text(r'$w_x =$ '+str(round(self.imagefitparams[0],1)) \
										+r' $\pm$ '+str(round(self.imagefitparams[1],1)))
		self.yfit_text.set_text(r'$w_y =$ '+str(round(self.imagefitparams[2],1)) \
										+r' $\pm$ '+str(round(self.imagefitparams[3],1)))
	...
	def OnStartScan(self,event):
		...
		if self.SaveEachImage:
			...
			SaveFileDialog = wx.FileDialog(self,"Autosave each image", "/media", "bp_scan_images_",
					"Pickle files (*.pkl)|*.pkl", wx.FD_SAVE)
		...
		self.scanning = True
		positions_array = np.arange(DialogOptions.scan_start_pos,
				DialogOptions.scan_stop_pos+DialogOptions.step_size,
				DialogOptions.step_size)
		...
		#after scan complete - if it hasn't been cancelled
			...
			ZXY = zip(self.xposdata, self.xwidthdata, self.xwidtherr, self.ywidthdata, self.ywidtherr)
			ZXY = sorted(ZXY, key=lambda f: f[0])
			pos, xw, xe, yw, ye = zip(*ZXY)
			...
			try:
				...
				try: 
					print('Trying fitting without using errorbars...',
					xfocus, xfocuserr = curve_fit(focussed_gaussian,pos, xw))#, sigma=xe)
				except:
	...
	def OnExport(self,event):
		SaveFileDialog = wx.FileDialog(self,"Save Output File", "/media", "beamprofiler_output",
		...
		if SaveFileDialog.ShowModal() == wx.ID_OK:
			...
			if os.path.isfile(profile_filename):
				...
				if OverwriteDialog.ShowModal() == wx.NO:
					...
					OverwriteDialog.Destroy()
					time.sleep(0.05)
			z,x,xe,y,ye = self.xposdata, self.xwidthdata, self.xwidtherr, \
						  self.ywidthdata, self.ywidtherr
			dataout = zip(z,x,xe,y,ye)
			...
			SaveMessage = wx.MessageDialog(self, \
				"Files created:\n\n  -- Beam profile data: "\
				+profile_filename+"\n -- Profile fit parameters: "+fits_filename, \
				"Files created", wx.OK|wx.ICON_INFORMATION)
	...
	def OnExportImage(self,event):
		...
		SaveFileDialog = wx.FileDialog(self,"Save Output File", "/media", "beamprofiler_output",
			"Pickle files (*.pkl)|*.pkl", wx.FD_SAVE|wx.FD_OVERWRITE_PROMPT)
	...
	def OnSaveFig(self,event):
		...
		SaveFileDialog = wx.FileDialog(self,message="Save Output File", defaultDir="/media", defaultFile="beamprofiler_figure",
	...
	def OnShutdown(self,event):
		...
		doshutdown = 0
		dlg = wx.MessageDialog(self, "Are you sure you want to shut down the device?", 
				"Confirm Shutdown", wx.OK|wx.CANCEL|wx.CANCEL_DEFAULT|wx.ICON_INFORMATION)
		if dlg.ShowModal() == wx.ID_OK:
			## do shutdown ...
			self.camera.cleanup()
			self.Destroy()
	...
	def TranslationStageCalibration(self):
		print('Calibrating..',
		dlg = wx.MessageDialog(self, "Click OK to start translation stage position calibration", \
						"Initial Calibration", wx.ICON_EXCLAMATION))
		...
		if retVal:
			dlg2 = wx.MessageDialog(self, "Translation stage position calibration completed", \
						"Initial Calibration Complete", wx.ICON_INFORMATION)	
			if dlg2.ShowModal():
				pass
			dlg2.Destroy()
		else:
			# error
			dlg2 = wx.MessageDialog(self, "Translation stage position calibration failed to complete - please check for hardware errors...", \
						"Initial Calibration Failed", wx.ICON_INFORMATION)
			if dlg2.ShowModal():
				pass
			dlg2.Destroy()
